...
// LSB3 implementation - returns XOR of last three bits
static int get_lsb3(unsigned char byte) {
    int bit1 = byte & 1;
    int bit2 = (byte >> 1) & 1;
    int bit3 = (byte >> 2) & 1;
    return bit1 ^ bit2 ^ bit3;  // XOR of last 3 bits
}

// Modify byte to match desired LSB3 value
static unsigned char modify_lsb3(unsigned char byte, int desired_bit) {
    int current_lsb3 = get_lsb3(byte);
    if (current_lsb3 != desired_bit) {
        // Toggle only LSB to minimize color changes
        byte ^= 1;
    }
    return byte;
}
    ...
    if (action) {
        ...
        for (int i = 0; i < msg_length; i++) {
            ...
            for (int bit = 0; bit < 8; bit++) {
                ...
                unsigned char new_byte = modify_lsb3(img_byte, desired_bit);
    ...
    // For decryption (if output is not .bmp)
        ...
        for (int i = 0; i < msg_length; i++) {
            ...
            for (int bit = 0; bit < 8; bit++) {
                unsigned char img_byte;
                fread(&img_byte, 1, 1, img_read);
                msg_byte |= (get_lsb3(img_byte) << (7-bit));
            }
