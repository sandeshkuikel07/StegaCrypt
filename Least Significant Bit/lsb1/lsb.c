...
// Modify byte to match desired LSB value
static unsigned char modify_lsb(unsigned char byte, int desired_bit) {
	if ((byte & 1) != desired_bit) {
		byte ^= 1;
	}
	return byte;
}
	...
	if (action) {
		...
		for (int i = 0; i < msg_length; i++) {
			...
			for (int bit = 0; bit < 8; bit++) {
				...
				unsigned char new_byte = modify_lsb(img_byte, desired_bit);
